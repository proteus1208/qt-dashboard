...
{
    //Do not set if the minimum value is greater than or equal to the maximum value
    ...
    //If the target value is not within the range value, reset the target value
    //If the value is less than the minimum value, take the minimum value, and if the value is greater than the maximum value, take the maximum value
    if (value < minValue) {
        setValue(minValue);
...
{
    //If the value is consistent with the current value, there is no need to deal with it
    if (value == this->value) {
        return;
    }
